-- Index obtained.user_id: used on every authenticated page load
create index if not exists obtained_user_id_idx on obtained (user_id);

-- Unique composite index on obtained: covers the specific item lookup in handleObtained()
-- and enforces no duplicate collection entries
create unique index if not exists obtained_unique_item_idx
  on obtained (user_id, eureka_set, category, color);
