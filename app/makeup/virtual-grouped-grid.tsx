...
import type { MakeupSet, MakeupVariant } from '@/lib/types/makeup'
import { STANDALONE_MAKEUP_SLUG } from '@/hooks/makeup'
import { useMakeupData } from '@/components/makeup/makeup-context'
import MakeupGroupHeader from './makeup-group-header'
...
type GroupRow =
  | {
      ...
      groupVariants: MakeupVariant[]
    }
  // The DISPLAYED (already filter-culled) variants for this row.
  | { kind: 'cards'; key: string; variants: MakeupVariant[] }
...
export default function VirtualGroupedGrid({
  ...
  const { makeupSets, onBatchToggleObtained } = useMakeupData()
  ...
  const rowModel = useMemo<GroupRow[]>(() => {
    ...
    for (const set of sets) {
      // `set.makeup_variants` is already filter-culled (e.g. the missing filter
      ...
      const fullSet = makeupSets.find((s) => s.id === set.id) ?? set
      const baseSlug = set.slug

      for (const evolution of [null, ...set.evolutions]) {
        const stateSlug = evolution?.slug ?? baseSlug
        const inState = (v: { makeup_set: string | null }) => v.makeup_set === stateSlug
        ...
        const groupVariants = fullSet.makeup_variants.filter(inState)
