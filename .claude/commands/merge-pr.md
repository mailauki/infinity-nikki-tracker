...
5. **Switch to main and pull** — `git checkout main && git pull --rebase`
6. **Safely clean up merged local branches** — run `git cleanup-merged --yes`. This deletes local branches whose PR is MERGED *and* whose commits are all in origin/main; it keeps (with a warning) any branch that still has stranded commits. Do **not** use bare `git branch -D` — the cleanup command exists specifically to avoid dropping post-squash commits.
