...
export default function CarouselImageUpload({
  ...
  const uploadImage: React.ChangeEventHandler<HTMLInputElement> = async (event) => {
    try {
      ...
      const filePath = `outfit_set_carousel_images/${outfitSetSlug}/${crypto.randomUUID()}.${fileExt}`
      ...
      const { data: inserted, error: dbError } = await supabase
        ...
        .insert({ outfit_set: outfitSetSlug, image_url: data.publicUrl, sort_order: nextOrder })
      ...
      enqueueSnackbar('Error uploading carousel image!', { variant: 'error' })
    } finally {
      setUploading(false)
  ...
  const deleteImage = async (img: CarouselImage) => {
    try {
      const { error } = await supabase.from('outfit_set_carousel_images').delete().eq('id', img.id)
      if (error) throw error
      onChange(images.filter((i) => i.id !== img.id))
    } catch {
      enqueueSnackbar('Error deleting carousel image!', { variant: 'error' })
    }
  }
