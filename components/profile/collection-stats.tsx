...
function StatItemRow({ item }: { item: StatItem }) {
  const percentage = percent(item.obtained, item.total)
  return (
    <ListItem disablePadding sx={{ py: 1, gap: 1.5, alignItems: 'center' }}>
      <LazyAvatar src={item.imageUrl ?? undefined} sx={{ width: 40, height: 40 }} />
...
export default function CollectionStats({
  ...
  const colorSetItems: StatItem[] = sets.flatMap((set) =>
    set.colors.map((color) => {
      ...
      return { title: `${set.title} — ${color.title}`, imageUrl: variants[0]?.image_url, obtained, total }
    })
  )
  ...
  const categoryItems: StatItem[] = categories.map((cat) => {
    const variants = allVariants.filter((v) => v.category === cat.slug)
    const { obtained, total } = countObtained(variants)
    return { title: cat.title, imageUrl: cat.image_url, obtained, total }
  })
  ...
  const colorItems: StatItem[] = colors.map((color) => {
    const variants = allVariants.filter((v) => v.color === color.slug)
    const { obtained, total } = countObtained(variants)
  ...
  return (
    ...
    >
      <CollectionStatCard items={setItems} obtained={setsObtained} title="Sets" total={sets.length} />
      <CollectionStatCard items={variantItems} obtained={variantsObtained} title="Variants" total={variantsTotal} />
      <CollectionStatCard items={colorSetItems} obtained={colorSetsObtained} title="Color Sets" total={colorSetsTotal} />
      <CollectionStatCard
        items={categoryItems}
      ...
      />
      <CollectionStatCard items={colorItems} obtained={colorsObtained} title="Colors" total={colors.length} />
      <CollectionStatCard items={trialItems} obtained={trialsObtained} title="Trials" total={trials.length} />
    </Box>
  )
}
