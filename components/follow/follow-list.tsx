...
export default function FollowList({
  ...
  /** Fired when a row is followed/unfollowed, so a parent can adjust the viewer's own Following count. */
  ...
  // Local overlay of follow changes made here, so a row's button stays correct
  ...
  const [changed, setChanged] = useState<
    Map<string, { profile: FollowProfile; following: boolean }>
  >(new Map())
  ...
  const isFollowingProfile = (id: string) =>
    changed.get(id)?.following ?? viewerFollowingIds.has(id)
  ...
  // Flattened out of a nested ternary: searching replaces the body wholesale,
