...
`makeup_sets.order` and an evolution's `makeup_sets.outfit_set` are both derived, not authored. `order` is 1 for a base set and always 4 for an evolution — the outfit line's max evolution, the same order as the `outfit_sets` row it pairs with. A base set's pairing is the only one picked by hand; each evolution resolves to the order-4 row of that same line (`resolveEvolutionOutfitSet()`, which falls back to the line's highest evolution and then the base's own pairing rather than ever blanking a link out). `makeupSetOrder()` and `resolveEvolutionOutfitSet()` in `hooks/makeup.ts` own both rules. Consequences: the admin forms have no order field and show Associated Outfit read-only on an evolution; the DataGrid locks Order and locks Associated Outfit on evolution rows; every write in `app/admin/makeup/sets/actions.ts` derives both; and saving a base set cascades its pairing to its evolutions (`syncEvolutionOutfitSets()`), writing only the rows that actually differ so `trg_makeup_sets_updated_at` doesn't churn the admin "recently updated" ordering.
