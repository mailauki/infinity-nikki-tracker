...
**Filled surfaces are selected by `surface` + `level`, never `elevation`** — the same role/step split as the type scale. `surface` is the M3 role (`base` | `dim` | `bright` | `container`, defaulting to `base`) and `level` the step within it (`lowest` | `low` | `medium` | `high` | `highest`, defaulting to `medium`). Only `container` has steps in M3, so `level` is inert on the other three. `<Card level="high">` → `surface.containerHigh`; `MuiCard` defaults to `surface="container" level="low"`. `elevation` stays a plain MUI shadow number for `variant="elevation"` papers and has no effect on a filled one. The variants are registered broadest-first (bare `filled` → role → role + level) so the more specific rule wins. Note `surface`/`level` do reach the DOM as inert attributes — MUI gives no theme-level `shouldForwardProp` hook.
